...
if ( !class_exists( 'Bkap_Multiple_Time_Slots' ) ) {
	...
	class Bkap_Multiple_Time_Slots {
		...
		function bkapmts_print_hidden_lockout( $product_id, $booking_settings ) {
		...
		function bkapmts_front_side_scripts_css() {
			...
			if ( isset( $booking_settings[ 'booking_enable_date' ] ) && $booking_settings[ 'booking_enable_date' ] == 'on' ) {
				wp_enqueue_style( 	'multiple-timeslot',
		...
		public function bkapmts_update_settings() {
			...
			$bkap_rental_version = get_option( 'bkap_multiple_time_slots_db_version' );
			
			if ( $bkap_rental_version != BKAPMTS_VERSION ) {
		...
		function bkapmts_time_load_ajax() {
		...
		function bkapmts_menu() {
			$page = add_submenu_page( 	'edit.php?post_type=bkap_booking',
										__( 'Activate Multiple Timeslot License', 'woocommerce-booking' ),
										__( 'Activate Multiple Timeslot License', 'woocommerce-booking' ),
										'manage_woocommerce',
		...
		function bkapmts_slot_function() {
		...
		function bkapmts_slot_type( $product_id, $booking_settings = array() ) {
		...
		function bkapmts_show_field_settings( $product_id ) {
			...
			<div id="booking_time_slot">
				...
				<div style="max-width:60%;display:inline-block;margin-left:10%;">
					...
					if ( isset( $booking_settings['booking_enable_multiple_time'] )
						&& $booking_settings['booking_enable_multiple_time'] == "multiple"
					) {
						$enable_time 	= "checked";
						$enabled_time 	= "";
					}
					...
					<input 	type="radio"
							...
							value="single"
							<?php echo $enabled_time = "checked";?>><?php _e( 'Single&nbsp&nbsp&nbsp&nbsp&nbsp;', 'woocommerce-booking' );?>
						</input>
					<input 	type="radio"
							id="booking_enable_time_radio"
							name="booking_enable_time_radio"
							value="multiple"<?php echo $enable_time;?>><?php _e( 'Multiple', 'woocommerce-booking' );?>
						</input>
                </div>
                ...
                <div style="width:10%;display:inline-block;margin-left:10%;">
					<img 	class="help_tip"
							width="16"
							height="16"
							data-tip="<?php _e('Enable Single to select single timeslot on product page or Enable Multiple to select multiple timeslots on product page.', 'woocommerce-booking');?>"
		...
		function bkapmts_product_settings_save( $booking_settings, $product_id ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_print_fields( $product_id, $booking_settings ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_show_multiple_time_price( $product_id, $booking_settings, $product, $booking_date, $variation_id, $gf_options, $resource_id ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_add_cart_item_data( $cart_arr, $product_id, $variation_id ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_get_item_data( $other_data, $cart_item ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_order_item_meta( $values, $order ) {
			global $wpdb;
		 ...
		 */
		function mts_quantity_check( $product_id, $booking_date, $booking_time_slot, $quantity ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_quantity_prod( $POST, $post_id ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_quantity_check( $value ) {
		 ...
		 * @since 1.0
		 */
		function bkapmts_cancel_order( $order_id, $item_value, $booking_id ) {
